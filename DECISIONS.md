...
## ADR-007 — Interim authentication via Atlassian OAuth (3LO); Entra/Azure AD deferred

**Status:** Accepted (2026-06-16), pilot scope

**Context.** The pilot shipped with a single shared-password gate — every session's `accountId` was the literal `"pilot-user"`, so the app could not tell users apart and the (already-designed) `UserRole` table and `authorize()` middleware sat unused. Stakeholders need differentiated roles now, but the production-correct identity source (Azure AD / Entra OIDC SSO) is blocked on IT provisioning. All pilot users have Atlassian accounts on the customer tenant.

**Decision.** Split authentication from authorization. For **authentication**, add **Atlassian OAuth 2.0 (3LO)** — "Log in with Atlassian" (scope `read:me`); the returned `account_id` is the same value Jira uses, so it maps directly onto `UserRole.atlassianAccountId`. Mint our own short-lived session JWT after the OAuth round-trip. Keep the shared password as a break-glass that logs in as a configured `SUPER_ADMIN_ACCOUNT_IDS` entry. For **authorization**, enforce roles at the single `/api/invoke` dispatch boundary via a key→roles `PERMISSIONS` map (read / execute / author / admin tiers, fail-closed), mirrored in the frontend for UI gating. A Super Admin panel manages role assignments.
