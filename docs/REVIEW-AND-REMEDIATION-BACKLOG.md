...
- [x] **✅ DONE — Fixed responsive overflow.** The real culprit was the non-wrapping header (6 nav tabs + account menu), not the body (which already stacks at ≤980px). Added `flex-wrap` to the header, nav, and list toolbar ([theme.css](../forge-app/static/frontend/src/theme/theme.css)). Verified 0 horizontal overflow at 600 / 768 / 980 / 1280px (was 58px at 768), no regression on wide desktop. *(Sub-~600px phone widths still cramp the fixed 280px sidebar — out of scope for this desktop/tablet tool.)*
- [x] **✅ DONE — Deep linking / URL routing.** Tab-level (`#dashboard`, `#runs`, …) **and per-test-case** (`#repository/<caseId>`): loading the link opens the case in its folder; a "🔗 Copy link" button in the editor produces the shareable URL ([App.tsx](../forge-app/static/frontend/src/App.tsx), [RepositoryView.tsx](../forge-app/static/frontend/src/features/repository/RepositoryView.tsx)). Linkable/bookmarkable and survives a refresh. *(Run-level deep links could follow the same pattern.)*
...
- [x] **✅ DONE — Attachment download with filename.** The execution runner now has a ⬇ button per attachment that saves with the original filename ([ExecutionRunner.tsx](../forge-app/static/frontend/src/features/runs/ExecutionRunner.tsx)), alongside the existing open-in-new-tab. (Storage stays base64-in-Postgres until the Blob migration in the Azure doc.)
